...
const PrescriptionForm = () => {
  ...
  const handleAddTodo = () => {
    if (medicine.trim() !== '' && dosage.trim() !== '' && description.trim() !== '') {
  ...
  return (
    <div>
      ...
      <div className="doc-form-container">
        ...
        <button onClick={handleAddTodo} className="doc-btn-add">Add Medicine</button>

        <ul className="doc-todo-list">
          {todoList.map((todo, index) => (
            <li key={index} className="doc-todo-item">
              <div>
                <strong>Medicine:</strong> {todo.medicine}, <strong>Dosage:</strong> {todo.dosage}, <strong>Description:</strong> {todo.description}
              </div>
              <button onClick={() => handleDeleteTodo(index)} className="doc-btn-delete">Delete</button>
            </li>
          ))}
        </ul>

        {todoList.length > 0 && !isUploaded && (
          <button onClick={handleUpload} className="doc-btn-upload">Upload Prescription</button>
        )}
